...
- model stands for which model you want to take into consideration:
  The Mixed Linear Model (MLM) is one of the most effective methods for controlling       false positives in GWAS. This model simultaneously incorporates both population         structure and cryptic relationship (Yu et al. Nature Genetics, 2006, 38: 203-208)
...
- Manhattan plots visualize GWAS results by plotting genomic SNP positions on the X-axis   and the negative logarithm of P-values from the F-test for assessing SNP-trait          associations on the Y-axis. Prominent peaks in the plot indicate significant genomic    regions linked to the trait
- The QQ-plot in GWAS assesses the model's ability to account for population structure    and familial relatedness by plotting the negative logarithms of P-values against their   expected values under the null hypothesis, with most points expected to lie on the      diagonal line, while deviations from this line, primarily in the upper right section,   suggest potential trait associations and spurious associations related to insufficient   model correction for such factors
- Association Table
  The GWAS result table provides a detailed summary of appropriate GWAS results. The      rows display theresults for each SNP above the user-specified minor allele frequency    threshold. The SNPs sorted by their
  position in the chromosome. The H&B P.Value is FDR adjusted P values by H&B method. N



References:
- Wang, J. & Zhang, Z. GAPIT Version 3: Boosting Power and Accuracy for Genomic            Association and Prediction.Genomics. Proteomics Bioinformatics 19, 1–12 (2021).
